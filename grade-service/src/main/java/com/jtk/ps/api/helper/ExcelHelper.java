...
public class ExcelHelper {
  ...
  // private static Comment createComment(String keterangan,Sheet sheet, Workbook
  // workbook, Integer col, Integer row) {
  ...
  // workbook.getCreationHelper().createRichTextString("keterangan");
  ...
  public static ByteArrayInputStream recapSeminartoExcelByType(List<SeminarValueParticipantDto> list, String sheetName,
      List<SeminarCriteria> listCriteria) {
      ...
      for (int col = 0; col < headerNames.length; col++) {
        if (col < 3) {
          ...
          sheet.addMergedRegion(
              new CellRangeAddress(0, 1, col + listCriteria.size() - 1, col + listCriteria.size() - 1));
          cell.setCellStyle(boldCellStyle);
        }
      }
  ...
  public static ByteArrayInputStream recapSeminartoExcel(List<SeminarCriteria> listCriteria,
      List<List<SeminarValueParticipantDto>> listValues, List<SeminarTotalValueDto> listTotal) {
    String sheetName;
    try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
      // style
      CellStyle boldCellStyle = workbook.createCellStyle();
  ...
  public static void rowHeaderPenguji(Sheet sheet, CellStyle boldCellStyle, List<SeminarCriteria> listCriteria,
      List<SeminarValueParticipantDto> listValues) {
  ...
  public static ByteArrayInputStream recapCoursetoExcelByType(String sheetName, List<RecapitulationCourseDto> recap,
      List<List<ComponentAndCriteriasDto>> listCriterias) {
    String[] headerNames = { "No", "NIM", "Nama", "Akhir" };
    String[] headerContentNames = { "Lain-lain Teori", "Lain-lain Praktek", "UTS Teori", "UTS Praktek", "UAS Teori",
        "UAS Praktek" };
    try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
      Sheet sheet = workbook.createSheet(sheetName);
      // style
      ...
      for (int i = 0; i < recap.get(0).getParticipant_data().size(); i++) {
        ...
        for (int j = participant.getComponent_data().size() - 1; j >= 0; j--) {
          ...
          for (int k = 0; k < merge; k++) {
            row.createCell(col++)
                .setCellValue(participant.getComponent_data().get(j).getCriteria_data().get(k).getValue());
          }
          if (merge == 0) {
  ...
  public static ByteArrayInputStream recapCoursetoExcel(List<RecapitulationCourseDto> recap,
      List<List<ComponentAndCriteriasDto>> listCriterias) {
    String[] headerNames = { "No", "NIM", "Nama", "Akhir" };
    String[] headerContentNames = { "Lain-lain Teori", "Lain-lain Praktek", "UTS Teori", "UTS Praktek", "UAS Teori",
        "UAS Praktek" };
    ...
    try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
      ...
      for (int i = 0; i < recap.size(); i++) {
        ...
        rowCourses(sheet, boldCellStyle, listCriterias.get(i), headerNames, headerContentNames,
            recap.get(i).getParticipant_data());
      }
  ...
  private static void rowCourses(Sheet sheet, CellStyle boldCellStyle, List<ComponentAndCriteriasDto> criterias,
      String[] headerNames, String[] headerContentNames, List<RecapitulationParticipantDto> participants) {
    Row headerRow = sheet.createRow(0);
  ...
  public static ByteArrayInputStream recapEtprotoExcel(List<EtproRecapitulationResponse> data){
      String sheetName;
      if(data.get(0).getParticipantDto().getProdi_id() == 0){
        sheetName = "Etika Profesi";
      }else{
        sheetName = "Etika dan Aspek Legal Teknologi Informasi";
      }
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
          // style
          CellStyle boldCellStyle = workbook.createCellStyle();
          ...
          Sheet sheet = workbook.createSheet(sheetName);
          List<EtproValueDto> dataAspect = data.get(0).getValues();

          ArrayList<String> headerEtpro = new ArrayList<>();
          headerEtpro.add("No");
          headerEtpro.add("NIM");
          headerEtpro.add("Nama");
          for (int i = 0; i < dataAspect.size(); i++) {
            headerEtpro.add(dataAspect.get(i).getAspect().getAspekName());
          }
          headerEtpro.add("Total");

          Row headerRow = sheet.createRow(0);
                
          for (int col = 0; col < headerEtpro.size(); col++) {
            Cell cell = headerRow.createCell(col);
            cell.setCellValue(headerEtpro.get(col));
            cell.setCellStyle(boldCellStyle);
          }
          ...
          for (int i = 0; i < data.size(); i++) {
            ...
            row.createCell(0).setCellValue(i+1);
            row.createCell(1).setCellValue(data.get(i).getParticipantDto().getNim());
            row.createCell(2).setCellValue(data.get(i).getParticipantDto().getName());
            for(int j = 0; j < data.get(i).getValues().size(); j++){
              row.createCell(3+j).setCellValue(data.get(i).getValues().get(j).getValue());
            }
            row.createCell(3+data.get(i).getValues().size()).setCellValue(data.get(i).getTotal());
          }
