...
public interface SeminarFormRepository extends JpaRepository<SeminarForm,Integer> {
    ...
    @Query(value = "select a.* from seminar_form a join participant b on b.id = a.participant_id where b.year = :year and b.prodi_id = :prodiId", nativeQuery = true)
    List<SeminarForm> findAllByYearAndProdi(Integer year,Integer prodiId);
    ...
    @Query(value = "SELECT case WHEN ( SELECT COUNT(*) FROM seminar_form  a join participant p on a.participant_id = p.id  WHERE YEAR(a.date_seminar) = :year AND a.is_finalization = 0 and p.prodi_id = :prodiId) = 0 THEN 1 ELSE 0 END AS is_all_finalized", nativeQuery = true)
    Integer isAllFinalization(Integer year, Integer prodiId);
